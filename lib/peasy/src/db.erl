...
%% To do this, peer records are overwritten or deleted in most cases (when the peer
...
-export([announce/1, torrent_peers/2, torrent_status/1, torrents_status/0, dump/0]).
...
torrents_status() ->
	gen_server:call({global, ?MODULE}, statuses).
...
handle_call({status, InfoHash}, _From, _State) ->
	...
	case mnesia:transaction(F) of
		{atomic, []} ->
			{reply, {torrent_status,0,0,0}, _State};
...
receive_announce(#peer{last_event=completed, peer_key={InfoHash, _PeerId}}=Peer) ->
	fun() ->
		case mnesia:read({torrent, InfoHash}) of
			[] -> mnesia:write(#torrent{complete=1, downloaded=1, info_hash = InfoHash});
...
%% Peer is just announcing and is a seeder
%% create or update the peer (if update, don't touch last_event)
receive_announce(#peer{peer_key={InfoHash, _PeerId}, left=0}=Peer) ->
	fun() ->
		case mnesia:read({torrent, InfoHash}) of
			[] -> mnesia:write(#torrent{info_hash = InfoHash, complete=1});
			_Any -> nil
		end,
...
%% create or update the peer (if update, don't touch last_event)
receive_announce(#peer{peer_key={InfoHash, _PeerId}}=Peer) ->
	fun() ->
		case mnesia:read({torrent, InfoHash}) of
			[] -> mnesia:write(#torrent{info_hash = InfoHash, incomplete=1});
			_Any -> nil
		end,
		case mnesia:read({peer, Peer#peer.peer_key}) of
			[] -> mnesia:write(Peer);
